    ...
    def train_one_epoch(self, epoch_num):
        ...
        try:
            for self.step, self.batch_data in self.pbar:
                log_step = self.step
                log_data = self.batch_data
                # if self.step in [348, 412, 490, 1116]: # 490, 1116, 1048, 550, 201]:
                #     continue
                self.train_in_steps(epoch_num, self.step)
                max_len = max(log_data[1].shape[0], max_len)
                self.print_details()
        except Exception as _:
            LOGGER.error('ERROR in training steps.')
            LOGGER.error(f'ERROR in {log_step}')
            LOGGER.error(log_data)
            LOGGER.error("Error shape:")
            LOGGER.error(log_data[1].shape)
            LOGGER.error("Max shape:")
            LOGGER.error(max_len)
            raise
